<?php
  ...
  class PageTabBlogCategories implements \ClicShopping\OM\Modules\HooksInterface
  {
    protected $app;
    protected $lang;
    protected $db;
    ...
    public function display()
    {
      ...
      if (isset($_GET['BlogCategoriesEdit'])) {

        $link_url = HTTP::getShopUrlDomain() . 'index.php?Blog&Categories&current=' . (int)$_GET['cID'];
...
<script>
$('#section_SEOReportApp_content').appendTo('#blogCategoriesTabs .tab-content');
$('#blogCategoriesTabs .nav-tabs').append('    <li class="nav-item"><a data-target="#section_SEOReportApp_content" role="tab" data-toggle="tab" class="nav-link">{$tab_title}</a></li>');
</script>
<!-- ######################## -->
<!--  End Report SEO APP      -->
<!-- ######################## -->
